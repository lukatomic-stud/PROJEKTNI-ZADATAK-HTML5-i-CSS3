...
// Dohvati vijest (user može uređivati samo svoju)
if ($role === "user") {
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$errors) {
        ...
        // Dodaj nove galerijske slike (opcionalno)
        if (isset($_FILES["gallery_images"]) && is_array($_FILES["gallery_images"]["name"])) {
            ...
            for ($i=0; $i<$count; $i++) {
                ...
                // mime_content_type koristi tmp_name, pa moramo dodati i full array
                $fileFull = [
                    "name" => $_FILES["gallery_images"]["name"][$i],
...
// brisanje pojedine slike (admin/editor; user samo svoje)
