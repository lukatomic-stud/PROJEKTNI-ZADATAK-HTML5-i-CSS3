<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] === "POST") {
      ...
      if (!$errors) {
          $stmt = $conn->prepare("
              INSERT INTO routes (sector_id, name, grade, length_m, bolts, route_type, description)
              VALUES (?, ?, ?, ?, ?, ?, ?)
          ");
...
<section class="content">
  ...
  <form class="contact-form" method="get" action="index.php">
    <input type="hidden" name="menu" value="37">
  ...
  <?php if ($sectorId <= 0): ?>
    <p>Odaberi penjalište i sektor gore, pa će se prikazati forma za unos smjera.</p>
  <?php else: ?>
    <form class="contact-form" method="post" action="index.php?menu=37&crag_id=<?php echo $cragId; ?>&sector_id=<?php echo $sectorId; ?>">
      ...
      <div class="form-row">
        <label for="grade">Težina (npr. 6a+, 7b, V5)</label>
      ...
      <div class="form-row">
        ...
        <input id="length_m" name="length_m" type="number" min="1" max="2000" value="<?php echo htmlspecialchars($lengthM); ?>">
      </div>

      <div class="form-row">
        <label for="bolts">Bolts</label>
        <input id="bolts" name="bolts" type="number" min="0" max="200" value="<?php echo htmlspecialchars($bolts); ?>">
      </div>
